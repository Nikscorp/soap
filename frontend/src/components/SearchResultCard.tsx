...
export function SearchResultCard({ result, onSelect }: Props) {
  ...
  return (
    <li>
      ...
      >
        <div className="flex h-[120px] w-[80px] flex-none items-center justify-center overflow-hidden rounded bg-surface-muted text-muted sm:h-[135px] sm:w-[90px]">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline gap-2">
            ...
            {year && <span className="shrink-0 text-sm text-muted">({year})</span>}
          </div>
          <div
            className="mt-1 flex items-center gap-1 text-xs font-semibold text-muted tabular-nums"
          ...
          {description ? (
            <p className="mt-2 line-clamp-3 text-sm leading-snug text-secondary sm:line-clamp-4">{description}</p>
          ) : (
            <p className="mt-2 text-sm text-muted italic">No description available.</p>
          )}
        </div>
      </button>
    </li>
