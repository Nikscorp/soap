...
- **TMDB response cache (`internal/pkg/tvmeta/cache.go`).** Typed LRU + TTL + `singleflight` wrappers around `TVShowDetails`, `TVShowAllSeasonsWithDetails`, and `SearchTVShows`. Cached values are pointers (`*TvShowDetails`, `*AllSeasonsWithDetails`, `*TVShows`) shared by every concurrent reader and **read-only** — same contract as `featuredExtras.view()`. The `all_seasons` cache holds the fully-assembled `/id/{id}` response with IMDb episode-rating overrides already applied, so warm hits skip both the season fan-out and the override pass; `search` holds the popularity-sorted TMDB result directly (no per-call mutation). Errors are never cached, so transient TMDB failures retry on the next request; size or TTL = 0 turns a method's cache into a pass-through (used by tests passing a zero `CacheConfig`).
