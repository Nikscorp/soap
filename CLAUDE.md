...
Cross-cutting: **`internal/pkg/lrucache`** is the shared cache harness (generic `Cache[K, V]` over `expirable.LRU` + `singleflight` + Prometheus metrics, with pass-through-on-zero-config). Both the TMDB response cache (`internal/pkg/tvmeta/cache.go`, family `tvmeta_cache_*`) and the `/img` bytes cache (`internal/app/lazysoap/img_cache.go`, family `lazysoap_img_cache_*`) are thin typed wrappers over it. Construct one `*lrucache.Metrics` per family via `lrucache.NewMetrics(reg, namePrefix, helpSubject)`, then `lrucache.New[K, V](method, size, ttl, m)` per cache.

Three pieces of non-obvious behavior to keep in mind when editing:

- **Featured-pool cache (`featured.go`).** TMDB popular shows ∪ operator-curated extras are resolved at startup and refreshed by a background goroutine (`runFeaturedPoolRefresh`, kicked off from `Server.Run`). Reads use `atomic.Pointer[[]featuredItem]` — copy-on-write swap, no locks on the request path. The cached slice is shared and **read-only**; never mutate or append to a slice returned by `featuredPool.view()`. The refresh goroutine is started async on purpose so a slow/down TMDB at boot can't block `ListenAndServe` (k8s liveness, fast restarts). On partial refresh failure (popular fetch fails *or* every extra ID fails) the prior pool is preserved; only the bootstrap publishes whichever side succeeded so `/featured` isn't stuck at 503. After every successful pool swap, `prewarmFeaturedImages` fetches every (poster, size) pair at the four SPA-declared sizes (`w185`, `w342`, `w500`, `w780`), bounded to 8 concurrent fetches via `errgroup.SetLimit`, and atomically replaces the pinned map in `featuredImgs` (see `/img` cache below). Per-poster failures are logged and dropped; if the parent ctx is canceled mid-prewarm the partial map is discarded and the prior pinned set is preserved (all-or-nothing publish). Prewarm calls `fetchPoster` directly — it does NOT write to the LRU `imgCache`, by design.
- **TMDB response cache (`internal/pkg/tvmeta/cache.go`).** Typed LRU + TTL + `singleflight` wrappers (over `internal/pkg/lrucache`) around `TVShowDetails`, `TVShowAllSeasonsWithDetails`, and `SearchTVShows`. Cached values are pointers (`*TvShowDetails`, `*AllSeasonsWithDetails`, `*TVShows`) shared by every concurrent reader and **read-only** — same contract as `featuredPool.view()`. The `all_seasons` cache holds the fully-assembled `/id/{id}` response with IMDb episode-rating overrides already applied, so warm hits skip both the season fan-out and the override pass; `search` holds the popularity-sorted TMDB result directly (no per-call mutation). Errors are never cached, so transient TMDB failures retry on the next request; size or TTL = 0 turns a method's cache into a pass-through (used by tests passing a zero `CacheConfig`). Metric family `tvmeta_cache_*{method=details|all_seasons|search}`.
- **`/img` bytes cache (`internal/app/lazysoap/img_cache.go`).** Two tiers, checked in order on every request:
   1. **`featuredImgs` (pinned shadow tier).** `atomic.Pointer[map[string]ImgCacheEntry]`, owned by the prewarmer, fully replaced on every featured-pool refresh. Has NO LRU, NO size cap, NO TTL, NO singleflight, NO metrics — by design. The whole point is that featured posters cannot be evicted by general `/id/{id}` traffic. Bounded by featured-pool size × `len(prewarmSizes)` (~120 entries on default config); each body still respects the 2 MiB cap so worst-case memory is predictable. The map is shared and read-only — never mutate an entry returned by `featuredImgs.lookup`. If you ever need to write here from outside the prewarmer, you're modeling something wrong; the tier's "static manifest" property is what makes it safe to skip singleflight.
   2. **`imgCache` (LRU+singleflight)**, the `internal/pkg/lrucache` harness keyed by `path + "|" + normalized-size`. Stores `ImgCacheEntry{body, contentType}` — bodies are shared and read-only, mutating one corrupts every in-flight response holding the same pointer. Capped at 2 MiB per body; only `200 OK` with `Content-Type: image/*` is cached. Errors map to `502` (or `404` for upstream-404) and aren't cached. Cache constructed in `cmd/lazysoap/main.go` and passed into `lazysoap.New`; nil/zero-config → pass-through. Metric family `lazysoap_img_cache_*{method=img}`.

   `imgProxyHandler` probes the pinned tier first and on miss falls through to `imgCache.GetOrFetch`. Pinned hits never touch the LRU and never round-trip TMDB; this is the load-bearing invariant — if you change the handler, preserve it. The two tiers share the same `imgCacheKey(path, size)` shape so a key produced by either side targets the right slot.
...
- `internal/app/lazysoap/server.go` — `Config` (server timeouts, `LAZYSOAP_FEATURED_*`, `DefaultBestQuantile`/`DefaultBestMinEpisodes` for the `/id/{id}` "default best" computation, `ImgClient` for the poster proxy, `ImgCache` — `LAZYSOAP_IMG_CACHE_SIZE` / `LAZYSOAP_IMG_CACHE_TTL` / `LAZYSOAP_IMG_BROWSER_MAX_AGE` — for the `/img` bytes cache).
