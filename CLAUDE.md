...
- **Featured-pool cache (`featured.go`).** TMDB popular shows ∪ operator-curated extras are resolved at startup and refreshed by a background goroutine (`runFeaturedPoolRefresh`, kicked off from `Server.Run`). Reads use `atomic.Pointer[[]featuredItem]` — copy-on-write swap, no locks on the request path. The cached slice is shared and **read-only**; never mutate or append to a slice returned by `featuredPool.view()`. The refresh goroutine is started async on purpose so a slow/down TMDB at boot can't block `ListenAndServe` (k8s liveness, fast restarts). On partial refresh failure (popular fetch fails *or* every extra ID fails) the prior pool is preserved; only the bootstrap publishes whichever side succeeded so `/featured` isn't stuck at 503. After every successful pool swap, `prewarmFeaturedImages` warms the `/img` cache for every (poster, size) pair at the four SPA-declared sizes (`w185`, `w342`, `w500`, `w780`), bounded to 8 concurrent fetches via `errgroup.SetLimit`; failures are logged and dropped, never returned.
