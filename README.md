...
Each request reshuffles, so refreshing the home page surfaces different shows. The response sets `Cache-Control: no-store`. Both halves of the pool — the popular list and the curated extras — are warmed into a single unified in-memory cache at startup and refreshed every `LAZYSOAP_FEATURED_EXTRAS_REFRESH_INTERVAL` (default 24h); the request path itself never round-trips TMDB. After every successful refresh, the `/img` cache is prewarmed for every poster in the new pool at the four renditions the SPA's responsive `srcset` declares (`w185`, `w342`, `w500`, `w780`), bounded to 8 concurrent fetches; failures are logged and dropped so a single bad poster doesn't stall the rest. On partial refresh failure (popular fetch fails *or* every extra ID fails) the prior pool is preserved rather than blanked. If the unioned pool would have fewer than `FEATURED_COUNT` items the handler returns 503 instead of a short list.
...
Successful responses are cached server-side: an in-process LRU keyed by `(path, normalized size)` holds up to `LAZYSOAP_IMG_CACHE_SIZE` (default `512`) poster bodies for `LAZYSOAP_IMG_CACHE_TTL` (default `168h`). Concurrent misses for the same key are collapsed into one upstream call via `singleflight`. Only `200 OK` responses with `Content-Type: image/*` and a body up to 2 MiB are cached; transport errors, non-image content-types, oversized bodies, and non-200 / non-404 statuses surface as `502` and are not cached. Upstream `404` is propagated as `404`. Successful responses also set `Cache-Control: public, max-age={LAZYSOAP_IMG_BROWSER_MAX_AGE}, immutable` (default `86400s`) — TMDB poster paths are content-addressed in practice so a long browser cache is safe; setting `LAZYSOAP_IMG_BROWSER_MAX_AGE=0` falls back to `Cache-Control: no-store`. Cache hits and misses emit identical headers so a hit is byte-identical to a fresh response. Hit / miss / error counts are exported on `/metrics` as `lazysoap_img_cache_hits_total` / `_misses_total` / `_errors_total` (label `method=img`).
