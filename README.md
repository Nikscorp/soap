...
Each request reshuffles, so refreshing the home page surfaces different shows. The response sets `Cache-Control: no-store`. Both halves of the pool — the popular list and the curated extras — are warmed into a single unified in-memory cache at startup and refreshed every `LAZYSOAP_FEATURED_EXTRAS_REFRESH_INTERVAL` (default 24h); the request path itself never round-trips TMDB. After every successful refresh, every poster in the new pool is prefetched at the four renditions the SPA's responsive `srcset` declares (`w185`, `w342`, `w500`, `w780`), bounded to 8 concurrent fetches; failures are logged and dropped so a single bad poster doesn't stall the rest. The prefetched bytes are pinned in a separate **unevictable** map (a "shadow" tier in front of the `/img` LRU) so general `/id/{id}` traffic can't push featured posters out under load — see the `/img` section below. On partial refresh failure (popular fetch fails *or* every extra ID fails) the prior pool is preserved rather than blanked, and if the prefetch is canceled mid-flight (e.g. shutdown) the prior pinned set is preserved (all-or-nothing publish). If the unioned pool would have fewer than `FEATURED_COUNT` items the handler returns 503 instead of a short list.
...
Server-side caching is a two-tier read-through. Each request first checks an **unevictable pinned map** (the "featured shadow tier"); on miss it falls through to the LRU below. The pinned map holds exactly the posters prefetched after the most recent featured-pool refresh — see the `/featured` section — and is fully replaced on every refresh, so dropped pool entries simply disappear at the next swap. The pinned tier has no size cap, no TTL, no LRU eviction, and emits no metrics: it is a static manifest rebuilt on a schedule rather than a dynamic cache. Bounded in practice by featured-pool size × 4 renditions (~120 entries on default config); each body is capped at 2 MiB so worst-case memory is predictable. The point of the tier is the explicit guarantee that a featured poster is never evicted by general `/id/{id}` traffic.

On a pinned miss, the request hits an in-process LRU keyed by `(path, normalized size)` that holds up to `LAZYSOAP_IMG_CACHE_SIZE` (default `512`) poster bodies for `LAZYSOAP_IMG_CACHE_TTL` (default `168h`). Concurrent misses for the same key are collapsed into one upstream call via `singleflight`. Only `200 OK` responses with `Content-Type: image/*` and a body up to 2 MiB are cached; transport errors, non-image content-types, oversized bodies, and non-200 / non-404 statuses surface as `502` and are not cached. Upstream `404` is propagated as `404`. Successful responses (from either tier) also set `Cache-Control: public, max-age={LAZYSOAP_IMG_BROWSER_MAX_AGE}, immutable` (default `86400s`) — TMDB poster paths are content-addressed in practice so a long browser cache is safe; setting `LAZYSOAP_IMG_BROWSER_MAX_AGE=0` falls back to `Cache-Control: no-store`. Pinned hits, LRU hits, and LRU misses all emit identical headers so a hit is byte-identical to a fresh response. LRU hit / miss / error counts are exported on `/metrics` as `lazysoap_img_cache_hits_total` / `_misses_total` / `_errors_total` (label `method=img`); the pinned tier is intentionally not metered.
...
| `LAZYSOAP_IMG_CACHE_SIZE` / `LAZYSOAP_IMG_CACHE_TTL` | `512` / `168h` | LRU bound and per-entry TTL for cached `/img` poster bodies. Setting either to `0` puts the cache in pass-through mode (every request goes upstream). |
| `LAZYSOAP_IMG_BROWSER_MAX_AGE` | `86400s` | `max-age` value emitted on `/img` `Cache-Control` headers. `0` switches the header to `no-store`. |
...
Concurrent cache misses for the same key are collapsed into a single TMDB call via `singleflight`. Errors are never cached: a transient TMDB failure is retried on the next request. Production builds populate the defaults shown in the env-var table above via `cleanenv`, so caching is on out of the box; setting any `*_SIZE` or `*_TTL` to zero disables the corresponding cache (the method falls through to plain TMDB calls), which is the mode used by tests that construct `tvmeta.CacheConfig{}` directly. The popular pool behind `/featured` lives in a separate per-process cache refreshed on `LAZYSOAP_FEATURED_EXTRAS_REFRESH_INTERVAL` (see [`/featured`](#get-featuredlanguage--random-featured-series-for-the-home-screen)) — the response cache here covers `/search` and `/id` only. Image responses from `/img/{path}` are cached separately by an in-process bytes cache (see [`/img/{path}`](#get-imgpathsize--tmdb-poster-proxy)). The cache is in-memory only, lost on restart, and observable via `tvmeta_cache_hits_total` / `_misses_total` / `_errors_total` (label `method=details|all_seasons|search`) on `/metrics`. The shared cache harness underneath `tvmeta` and `/img` lives in `internal/pkg/lrucache` (LRU + TTL + singleflight + Prometheus metrics, with pass-through-on-zero-config); the `/img` cache emits a parallel `lazysoap_img_cache_*` family with the same shape (`method=img`).
