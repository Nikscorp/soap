...
type RatingsProvider interface {
	// SeriesRating returns the rating for a series tconst (e.g. "tt0944947").
	// ok=false means the dataset has no entry; callers should fall back.
	SeriesRating(imdbID string) (rating float32, votes uint32, ok bool)
...
func (NoopRatingsProvider) SeriesRating(string) (float32, uint32, bool) {
	return 0, 0, false
}
